          ...
          <TabsContent>
            ...
            >
              <p className="text-slate-400 mb-2">
                Private Office This is required when, for example, the final
                text is not yet available. Dummy text is also known as 'fill
                text'. It is said that song composers of the past used dummy
                texts as lyrics. Private Office This is required when, for
                example, the final text is not yet available. Dummy text is also
                known as 'fill text'. It iss said that song composers of the
                past used dummy texts as lyrics. Private Office This is required
                when, for example, the final text is not yet available. Dummy
                text is also known as 'fill text'. It is said that song
                composers of the past used dummy texts as lyrics. Private Office
...
export const TabItem = ({ children, onClick, isActive }: any) => {
  const isActiveClass = isActive ? "bg-red-500 text-white" : "";
  return (
    <li role="presentation" className={`pt-1 pb-1 ${isActiveClass}`}>
      <button
...
export const TabsContentItem = ({ children, title, activeTab, id }: any) => {
  return (
    <div
      className={activeTab === id ? "overflow-auto max-h-96" : "hidden"}
      id="contacts"
      role="tabpanel"
    ...
    >
      <div className="mt-6">
        {title && (
          <h5 className="text-lg text-black font-semibold mb-4">{title}</h5>
        )}
        <>{children}</>
      </div>
    </div>
